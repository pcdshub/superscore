...
class TestTestBackend:
    def test_retrieve(self, linac_backend):
        assert linac_backend.get_entry(UUID("441ff79f-4948-480e-9646-55a1462a5a70")) is not None  # top-level Collection
        assert linac_backend.get_entry(UUID("be3d4655-7813-4974-bb10-19e4787f8a8e")) is not None  # Inner Collection
        assert linac_backend.get_entry(UUID("2c83a9be-bec6-4436-8233-79df300af670")) is not None  # Parameter
        with pytest.raises(EntryNotFoundError):
            linac_backend.get_entry(UUID("d3589b21-2f77-462d-9280-bb4d4e48d93b"))  # Doesn't exist

    def test_create(self, linac_backend):
        collision_entry = Parameter(uuid=UUID("5ec33c74-7f4c-4905-a106-44fbfe138140"))
        with pytest.raises(EntryExistsError):
            linac_backend.save_entry(collision_entry)

        new_entry = Parameter(uuid=UUID("8913b7af-830d-4e32-bebe-b34a4616ce79"))
        linac_backend.save_entry(new_entry)
        assert linac_backend.get_entry(UUID("8913b7af-830d-4e32-bebe-b34a4616ce79")) is not None

    def test_update(self, linac_backend):
        modified_entry = Collection(uuid=UUID("d5bade05-d992-4e44-87d8-0db2937209bf"), description="This is the new description")
        linac_backend.update_entry(modified_entry)
        assert linac_backend.get_entry(modified_entry.uuid) == modified_entry

        missing_entry = Collection(uuid=UUID("d3589b21-2f77-462d-9280-bb4d4e48d93b"))
        with pytest.raises(EntryNotFoundError):
            linac_backend.update_entry(missing_entry)

    def test_delete(self, linac_backend):
        entry = linac_backend.get_entry(UUID("2506d87a-5980-4470-b29a-63eea183f53d"))
        linac_backend.delete_entry(entry)
        with pytest.raises(EntryNotFoundError):
            linac_backend.get_entry(UUID("2506d87a-5980-4470-b29a-63eea183f53d"))

        entry = linac_backend.get_entry(UUID("aa11f29a-3e7e-4647-bfc9-133257647fb7"))
